...
class VLTransE(nn.Module):
    """
        Temporal matters. Sequence of the <subject, predicate, object> matters. this should be somehow
        encoded within the embeddings. As of currently, there is no such way of encoding this information
        in BERT
    """
    def __init__(self, cfg, in_channels, eps=1e-10):
        ...
        # Relation Predictor (Finetuning head)
    ...
    def to(self, device, **kwargs):
        super(VLTransE, self).to(device, **kwargs)
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        rel_sem_class_logits = self.text_encoder(torch.stack((subj_vert_labels, obj_vert_labels,), 1)) # (N, 2) for N proposal pairs

        # This returns a set of features

        '''sum up all scores'''
        rel_class_logits = rel_vis_class_logits + rel_spt_class_logits
...
def build_vltranse_model(cfg, in_channels):
    return VLTransE(cfg, in_channels)
